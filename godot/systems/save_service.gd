...
func _migrate_if_needed(save_dict: Dictionary) -> Dictionary:
	...
	## version up to CURRENT_SAVE_VERSION, applying each one.
	##
	## A save newer than CURRENT_SAVE_VERSION (player rolled back the app)
	## is loaded as-is with a warning; we don't downgrade the version field
